...
RUN curl -O -L https://download.qt.io/archive/qt/5.12/5.12.8/qt-opensource-linux-x64-5.12.8.run && \
  chmod 0711 qt-opensource-linux-x64-5.12.8.run && \
  ./qt-opensource-linux-x64-5.12.8.run --script qt-installer-noninteractive.qs --platform minimal --verbose
